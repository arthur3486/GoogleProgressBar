...
public class GoogleMusicDicesDrawable extends Drawable {
    ...
    private ProgressState[] mProgressStates;
    ...
    private class ProgressState {
        ...
        ProgressState(DiceSide side1, DiceSide side2) {
            this.side1 = side1;
            this.side2 = side2;
        }
    }
    ...
    private void init() {
        ...
        mProgressStates = new ProgressState[] {
                new ProgressState(DiceSide.ONE, DiceSide.THREE),
                new ProgressState(DiceSide.TWO, DiceSide.THREE),
                new ProgressState(DiceSide.TWO, DiceSide.SIX),
                new ProgressState(DiceSide.FOUR, DiceSide.SIX),
                new ProgressState(DiceSide.FOUR, DiceSide.FIVE),
                new ProgressState(DiceSide.ONE, DiceSide.FIVE)
        };
        mRotation = Rotation.LEFT;
    ...
    private void drawScaleX(Canvas canvas) {
        ...
        drawDiceSide(canvas, mProgressStates[mCurrentProgressState].side1, mScale > 0.1f);
        canvas.restore();
        ...
        canvas.concat(matrix);
        drawDiceSide(canvas, mProgressStates[mCurrentProgressState].side2, false);
        canvas.restore();
    }
    ...
    private void drawScaleY(Canvas canvas) {
        ...
        canvas.concat(matrix);
        drawDiceSide(canvas, mProgressStates[mCurrentProgressState].side1, false);
        canvas.restore();
        ...
        canvas.concat(matrix);
        drawDiceSide(canvas, mProgressStates[mCurrentProgressState].side2, mScale > 0.1f);
        canvas.restore();
    }
    ...
    private void setupAnimations() {
        ...
        objectAnimator.addListener(new Animator.AnimatorListener() {
            ...
            public void onAnimationEnd(Animator animation) {
                mScale = 0;
                mCurrentProgressState = ++mCurrentProgressState % mProgressStates.length;
                mRotation = mRotation.invert();
